...
//    val resEdges =
//      (for (e1 <- edges; e2 <- other.edges; if e1.compat(e2,shared))
//                     yield e1.join(e2,prod))            ++
//            edges.filter(_.act.intersect(shared).isEmpty) ++
//      other.edges.filter(_.act.intersect(shared).isEmpty)
    val resEdges = (for (e1 <- edges; e2 <- other.edges; if e1.compat(e2,shared))
                     yield e1.join(e2,prod))            ++
                   (for (e1 <- edges.filter(_.act.intersect(shared).isEmpty); loc <- other.locs) yield e1.independent(loc, prod,1)) ++
                   (for (e2 <- other.edges.filter(_.act.intersect(shared).isEmpty); loc <- locs) yield e2.independent(loc, prod,2))
