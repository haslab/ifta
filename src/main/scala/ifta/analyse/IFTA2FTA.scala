...
object IFTA2FTA {
  ...
  def apply(i: IFTA): FTA = {
    ...
    FTA(newLocs,iFTA.init,committed,iFTA.act.map(mkAct(_,iFTA.in,iFTA.out)),iFTA.clocks,iFTA.feats,newEdges,iFTA.cInv,iFTA.fm,iFTA.aps,iFTA.shortname)
  }
...
//        genCommitLocs(Edge(lloc,e.cCons,e.act - a,e.cReset,e.fe,e.to), lloc, nloc, es, co,ins,outs) match {
        genCommitLocs(Edge(lloc,CTrue,e.act - a,Set(),e.fe,e.to), lloc, nloc, es, co,ins,outs) match {
